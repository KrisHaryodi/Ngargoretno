...
if(empty($_SESSION['username']) AND empty($_SESSION['passuser'])){
  ...
  $querySimpan = mysqli_query($konek, "INSERT INTO tbl_artikel (artikel_judul, artikel_tgl, artikel_penulis, artikel_isi, artikel_foto) VALUES ('$judul', '$tgl', '$penulis', '$isi', '$namafile')");
      if($querySimpan){
        echo "<script> alert ('Data Berhasil Ditambahkan'); window.location = '$super_url'+'adminweb.php?module=artikel'; </script>";     
      }
      else{
        echo "<script> alert ('Data Gagal Ditambahkan'); window.location = '$super_url'+'adminweb.php?module=artikel';</script>";
      }
  }else{
