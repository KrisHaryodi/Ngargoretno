...
if (empty($_SESSION['username']) AND empty($_SESSION['passuser'])) {
    ...
    if(move_uploaded_file($lokasifile, $uploadfile)){
      ...
      if($querySimpan){
        echo "<script> alert ('Data Berhasil Ditambahkan'); window.location = '$super_url'+'adminweb.php?module=home';</script>";     
      }
      else{
        echo "<script> alert ('Data Gagal Ditambahkan'); window.location = '$super_url'+'adminweb.php?module=home';</script>";
      }
  }else{
        $querySimpan = mysqli_query($konek, "INSERT INTO tbl_profile (profile_judul, profile_isi, profile_foto) VALUES ('$judul', '$isi', '$namafile')");
    echo "<script> alert ('Data Berhasil Ditambahkan'); window.location = '$super_url'+'adminweb.php?module=home';</script>";
